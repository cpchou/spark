...
RUN wget -q -P /usr/local/spark-2.2.0-bin-hadoop2.7/jars/  https://cpchou0701.diskstation.me/jdk/graphframes-0.6.0-spark2.2-s_2.11.jar -O graphframes.zip
RUN python -m  pip install torch==1.4.0+cpu torchvision==0.5.0+cpu -f https://download.pytorch.org/whl/torch_stable.html
ENV PYTHONPATH=/usr/local/spark-2.2.0-bin-hadoop2.7/jars/graphframes.zip:/usr/local/spark-2.2.0-bin-hadoop2.7/python/lib/py4j-0.10.4-src.zip:/usr/local/spark-2.2.0-bin-hadoop2.7/python:PYSPARK_DRIVER_PYTHON=ipython
