...
RUN ["/bin/bash", "-c", "wget  -P /usr/local/spark-2.4.5-bin-hadoop2.7/jars/  https://github.com/microsoft/mssql-jdbc/releases/download/v8.2.0/mssql-jdbc-8.2.0.jre8.jar"]
RUN ["/bin/bash", "-c", "wget  -P /usr/local/spark-2.4.5-bin-hadoop2.7/jars/  https://jdbc.postgresql.org/download/postgresql-42.2.10.jar"]
RUN ["/bin/bash", "-c", "wget  -P /usr/local/spark-2.4.5-bin-hadoop2.7/jars/  http://search.maven.org/remotecontent?filepath=mysql/mysql-connector-java/5.1.32/mysql-connector-java-5.1.32.jar -O mysql-connector-java-5.1.32.jar"]
